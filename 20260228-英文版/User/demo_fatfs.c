...
void DemoFatFS(void)
{
	...
	while (1)
	{
		...
		{
			  UartProtocol_DataAnalyze(RX_Buffer, ch_num);
				rx_flag=0;
    ...
    if (res == FR_OK) {
        ...
        while ((res = f_readdir(&dir, &fno)) == FR_OK && fno.fname[0] != 0) {
            ...
            if (ext && strcmp(ext, ".txt") == 0) {
				...
				printf("page14.t8.txt=\"USB Drive Open OK\"\xff\xff\xff");
				
    } else {
        printf("page14.t8.txt=\"USB Drive Open Fail\"\xff\xff\xff");
    }
...
void CreateNewFile(unsigned char *dataBuf, uint16_t dataLen)
{
	...
	/* 从接收数据中解析日期 YYYYMMDD (dataBuf已指向帧头) */
	memcpy(currentDate, dataBuf + 5, 8);
	currentDate[8] = '\0';
	
	/* 挂载文件系统（提前挂载，供ReadFileNumber和后续写入共用） */
	...
	if ((writeCount == 0 || strcmp(txtDate, currentDate) != 0) && QDvalue == 1) {
		/* 每次创建新文件时，或日期变化时，都查找该日期的最大编号 */
		strcpy(txtDate, currentDate);
		txtFileNumber = ReadFileNumber(currentDate);
		txtFileNumber++;  /* 在最大编号基础上+1，没有文件时从1开始 */
		writeCount = 0;   /* 日期变化时重置写入次数 */
	}
	...
	if (result == FR_OK&&QDvalue==1)
	{
		...
		if (writeCount == 0) {
			const char csvHeader[] = "Date^Time^TOC^Cond1^No\r\n";
			result = f_write(&file, csvHeader, strlen(csvHeader), &bw);
		}
	 result = f_lseek(&file, f_size(&file));
		if (dataLen > 5) {  // 确保数据长度足够（头5字节）
    result = f_write(&file, dataBuf + 5, dataLen - 5, &bw);//写入数据内容(dataLen不含CRC)
}
if (result == FR_OK) {	
		/* 写入换行符 */
   const char newline[] = "\r\n";
	...
	if (result == FR_OK) {
		printf("page14.t0.txt=\"%03d.txt Write OK\"\xff\xff\xff", txtFileNumber);	
		}
}
	...
	{
		printf("page14.t0.txt=\"%03d.txt Write Fail\"\xff\xff\xff", txtFileNumber);
	}
...
void CreateNewCSVFile(unsigned char *dataBuf, uint16_t dataLen)
{
	...
	/* 挂载文件系统（提前挂载，供ReadCSVFileNumber和后续写入共用） */
	result = f_mount(FS_USB, &fs);
	if (result != FR_OK)
	{
		printf("挂载文件系统失败 (%d)\r\n", result);
	...
	if (result == FR_OK && CSVvalue == 1)
	{
		...
		result = f_lseek(&file, f_size(&file));
		if (dataLen > 5) {
			/* 解析数据并转换为CSV格式写入 (dataBuf已指向当前帧头) */
			char csvLine[64] = {0};
			uint16_t csvLen = CSVData_Analyze(dataBuf, csvLine);
			if (csvLen > 0) {
		...
		if (result == FR_OK) {
			...
			result = f_write(&file, newline, sizeof(newline) - 1, &bw);
			if (result == FR_OK) {
				printf("page14.t0.txt=\"%03d.csv Write OK\"\xff\xff\xff", csvFileNumber);
			}
		}
	...
	{
		printf("page14.t0.txt=\"%03d.csv Write Fail\"\xff\xff\xff", csvFileNumber);
	}
...
*	功能说明: 停止CSV下载时调用，将csv目录下所有CSV文件设置为只读
*	形    参：无
*	返 回 值: 无
*********************************************************************************************************
*/
void SetCSVFileReadOnly(void)
{
	...
	/* 打开csv目录 */
	result = f_opendir(&dir, "/csv");
	if (result == FR_OK) {
		/* 遍历csv目录下所有文件 */
		while ((result = f_readdir(&dir, &fno)) == FR_OK && fno.fname[0] != 0) {
			if (fno.fattrib & AM_DIR) continue;  /* 跳过子目录 */
			if (fno.fattrib & AM_RDO) continue;  /* 已经是只读的跳过 */
	...
	printf("page14.t1.txt=\"%d csv ReadOnly\"\xff\xff\xff", count);
...
/*
*********************************************************************************************************
*	函 数 名: USB_PrepareFileSystem
*	功能说明: U盘插入后立即调用，预先创建txt和csv目录，避免首次写入时耗时创建
*	形    参：无
*	返 回 值: 无
*********************************************************************************************************
*/
