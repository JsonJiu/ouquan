...
static uint8_t usb_fs_prepared = 0;  /* U盘文件系统是否已初始化标志 */
...
{
	usb_fs_prepared = 0;  /* U盘拔出，重置标志，下次插入重新创建目录 */
...
{
	/* U盘枚举成功后，只执行一次：创建txt和csv目录 */
	if (usb_fs_prepared == 0)
	{
		USB_PrepareFileSystem();
		usb_fs_prepared = 1;
	}
	return 0;
}
