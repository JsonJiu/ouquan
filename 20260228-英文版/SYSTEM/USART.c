	...
	while (1)
	{
		...
		if (buf[pHead] == 0x88 && buf[pHead + 1] == 0x67 && buf[pHead + 2] == 0x96)
		{
			...
			case 0x9BB7: // txt文件数据导出启动
				writeCount = 0;
				QDvalue = 1;
				printf("page14.t1.txt=\"TXT Download ON\"\xff\xff\xff");
				pHead += 4;				
				break;
			case 0x9BB8: // csv文件数据导出启动
				csvWriteCount = 0;
				CSVvalue = 1;
				printf("page14.t1.txt=\"CSV Download ON\"\xff\xff\xff");
				pHead += 4;
				break;
			case 0x5BB4: // 数据导出停止
		   		if(QDvalue == 1)
		   		{
					QDvalue = 0;
					printf("page14.t1.txt=\"TXT Download OFF\"\xff\xff\xff");
		   		}
		   		if(CSVvalue == 1)
		   		{
					CSVvalue = 0;
					SetCSVFileReadOnly();  /* 停止时设置CSV文件为只读 */
					printf("page14.t1.txt=\"CSV Download OFF\"\xff\xff\xff");
		   		}
				pHead += 4;
				break;
			case 0x3BB3: // 启动打印
				DYvalue = 1;
				pHead += 4;
				printf("page14.t1.txt=\"Print Start\"\xff\xff\xff");
				break;
			case 0x3BB4: // 停止打印
				DYvalue = 0;
				pHead += 4;
				printf("page14.t1.txt=\"Print Stop\"\xff\xff\xff");
				break;
			case 0x4BB1:								   // Page14数据控件写入U盘
			{
				...
				if(last_5E_pos > 0)
				{
					...
					{
						LED_FLAG_Run();
						printf("page14.t1.txt=\"CRC OK\"\xff\xff\xff");
						if (QDvalue == 1)
						{						
					...
					{
						printf("page14.t1.txt=\"CRC Error\"\xff\xff\xff");
					}
					pHead = crc_pos + 1;  // 跳过CRC(2字节)，是因为循环末尾还有pHead++
				}
				else
				{
					printf("page14.t1.txt=\"Format Error\"\xff\xff\xff");					
				}
			}
				break;	
...
void shujuneiruDATA_Analyze(unsigned char *buf, uint16_t buf_len)
{
	...
	if(field_count >= 3)
	{
		...
		// 发送到打印机
		RS232_Printf("\r\n--------------------------------\r\n");
		RS232_Printf("Time: %04d-%02d-%02d %02d:%02d:%02d\nTOC: %.2f ppb\nCond: %.4f us/cm\r\n",
					 Data.year, Data.month, Data.day,
					 Data.hour, Data.minute, Data.second,
					 Data.concentration_ppb, Data.conductivity);
		if (i == 1)
		{
			printf("page14.t2.txt=\"First Print OK\"\xff\xff\xff");
		}
		else
		{
			printf("page14.t2.txt=\"Print OK\"\xff\xff\xff");
		}
	}
}
