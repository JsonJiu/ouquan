...
版本：V4.0
编 写 人：WEJ
注    意：本次增加CSV文件创建，并修改文件名带日期，现在程序同时支持txt文件和csv文件的写入
***********************************************************************/
...
uint16_t CSVData_Analyze(unsigned char *buf, char *csvOut)
{
	...
	{
		// 解析时间
		uint16_t year = parse_4digit(&buf[Index_time_idx]);
		uint8_t month = parse_2digit(&buf[Index_time_idx + 4]);
		uint8_t day = parse_2digit(&buf[Index_time_idx + 6]);
		uint8_t hour = parse_2digit(&buf[Index_time_idx + 8]);
		uint8_t minute = parse_2digit(&buf[Index_time_idx + 10]);
		uint8_t second = parse_2digit(&buf[Index_time_idx + 12]);
		
		// 解析浓度
		uint8_t ppb_len = Index_5EFrame2 - Index_Datappb;
		char ppb_str[16] = {0};
		memcpy(ppb_str, &buf[Index_Datappb], ppb_len);
		
		// 解析电导率
		uint8_t conductivity_len = Index_CRC - Index_diandao;
		char conductivity_str[16] = {0};
		memcpy(conductivity_str, &buf[Index_diandao], conductivity_len);
		
		// 格式化为CSV行: "MM/DD/YYYY HH:MM:SS,浓度,电导率"
