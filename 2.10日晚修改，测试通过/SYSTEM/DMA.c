...
{  
     ...
     // 清零发送缓冲区，避免发送垃圾数据
     memset(TX_Buffer, 0, USART6_DMA_TX_BUFFER_MAX_LENGTH);
     ...
     while (DMA_GetCmdStatus(DMA2_Stream6) != DISABLE);//等待DMA2_Stream6可配置 
      ...
      DMA_InitStructure.DMA_BufferSize = 0;//初始化时设为0，避免发送垃圾数据
...
{	
	// 检查参数有效性
	if (nSendCount == 0 || nSendCount > USART6_DMA_TX_BUFFER_MAX_LENGTH)
	{
		return;  // 参数无效，直接返回
	}
	...
	// 设置传输数据量
	DMA_SetCurrDataCounter(DMA2_Stream6, nSendCount);
	...
	while(1)
	{
		if(DMA_GetFlagStatus(DMA2_Stream6, DMA_FLAG_TCIF6) != RESET)  // 等待DMA2_Stream6传输完成
		{ 
			DMA_ClearFlag(DMA2_Stream6, DMA_FLAG_TCIF6);  // 清除DMA2_Stream6传输完成标志
			break; 
		}
	}
}
