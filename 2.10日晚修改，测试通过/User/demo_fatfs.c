...
if (result == FR_OK) {	
   ...
   const char newline[] = "\r\n";
	...
	if (result == FR_OK) {
		printf("page14.t0.txt=\"%03d.txt文件写入成功\"\xff\xff\xff", txtFileNumber);	
		}
}
	...
	{
		printf("page14.t0.txt=\"%03d.txt文件写入失败\"\xff\xff\xff", txtFileNumber);
	}
...
void CreateNewCSVFile(unsigned char *dataBuf, uint16_t dataLen)
{
	...
	if (result == FR_OK && CSVvalue == 1)
	{
		...
		if (result == FR_OK) {
			...
			result = f_write(&file, newline, sizeof(newline) - 1, &bw);
			if (result == FR_OK) {
				printf("page14.t0.txt=\"%03d.csv文件写入成功\"\xff\xff\xff", csvFileNumber);
			}
		}
	...
	{
		printf("page14.t0.txt=\"%03d.csv文件写入失败\"\xff\xff\xff", csvFileNumber);
	}
...
void SetCSVFileReadOnly(void)
{
	...
	printf("page14.t1.txt=\"%d个csv文件已设为只读\"\xff\xff\xff", count);
