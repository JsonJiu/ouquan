...
版本：V3
编 写 人：WEJ
注    意：本次修改去除了读取U盘的数据打印，直接通过串口的数据进行打印。
***********************************************************************/
	...
	while (1)
	{
		...
		if (buf[pHead] == 0x88 && buf[pHead + 1] == 0x67 && buf[pHead + 2] == 0x96)
		{
			...
			case 0x4BB2:								   // 数据控件数据写入U盘
				...
				if (DYvalue == 1 && value == crc_value)
				{
					printf("page17.t1.txt=\"检验正确\"\xff\xff\xff");
					shujuneiruDATA_Analyze(&buf[pHead]);
					LED_FLAG_Run();
					if (QDvalue == 1)
					{
						i++;
						CreateNewFile(); /* 创建一个新U盘文件,写入一个字符串 */
					}
				}
				else
				{
				...
				pHead += 32;
				break;
				case 0x4BB3:								   // 数据控件数据写入SD卡
				crc_value = CRC16_MODBUS(&buf[pHead], 31); // 计算发来数据的检验码
				ptr = &buf[pHead] + 31;					   // 指向数组的倒数第二个元素
				value = *((uint16_t *)ptr);				   // 读取发来的校验码
				if (value == crc_value)
				{
					printf("page0.t1.txt=\"检验正确\"\xff\xff\xff");
					LED_FLAG_Run();
					CreatesdNewFile(); /* 创建一个新SD文件,写入一个字符串 */					
				}
				else
				{
					printf("page17.t1.txt=\"检验错误\"\xff\xff\xff");
